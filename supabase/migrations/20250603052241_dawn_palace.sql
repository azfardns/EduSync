...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'profiles' AND column_name = 'enrollment_completed') THEN
    ALTER TABLE profiles ADD COLUMN enrollment_completed boolean DEFAULT false;
  END IF;

  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'profiles' AND column_name = 'faculty_id') THEN
    ALTER TABLE profiles ADD COLUMN faculty_id uuid REFERENCES faculties(id) ON DELETE SET NULL;
  END IF;

  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'profiles' AND column_name = 'program_id') THEN
    ALTER TABLE profiles ADD COLUMN program_id uuid REFERENCES programs(id) ON DELETE SET NULL;
  END IF;

  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'profiles' AND column_name = 'enrollment_locked') THEN
    ALTER TABLE profiles ADD COLUMN enrollment_locked boolean DEFAULT false;
  END IF;

  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'profiles' AND column_name = 'year_of_study') THEN
    ALTER TABLE profiles ADD COLUMN year_of_study integer CHECK (year_of_study BETWEEN 1 AND 4);
  END IF;
